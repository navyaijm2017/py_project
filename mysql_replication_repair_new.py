...
master = {
    ...
    'ssh_user': 'root',
    'ssh_pass': 'mysql',
    'mysqlbinlog_cmd': '/usr/bin/mysqlbinlog56',
    'binlog_file_path': '/var/lib/mysql/'
}
...
class Dbapi(object):
    def __init__(self, host, user, password, port, database=None):
        #self.conn = MySQLdb.connect(host=host, user=user, passwd=password, port=int(port),database=database, charset='utf8')
        self.conn = MySQLdb.connect(host=host, user=user, passwd=password, port=int(port), charset='utf8')
        if database:
